...
# Create PagerDuty users for the Authentication team
resource "pagerduty_user" "authentication_team_users" {
  for_each = can(var.teams["authentication_team"]) ? var.teams["authentication_team"].members : {}
  name  = each.value.name
  email = each.value.email
}
...
# Create PagerDuty team memberships for the Authentication team
resource "pagerduty_team_membership" "authentication_team_memberships" {
  for_each = can(var.teams["authentication_team"]) ? var.teams["authentication_team"].members : {}

  user_id = pagerduty_user.authentication_team_users[each.key].id
  team_id = pagerduty_team.teams["authentication_team"].id
}
...
resource "pagerduty_escalation_policy" "sre_escalation_policy" {
  ...
  name      = "SRE Team Escalation Policy"
  num_loops = 2
...
resource "pagerduty_escalation_policy" "engineering_escalation_policy" {
  ...
  name      = "Growth Team Escalation Policy"
  num_loops = 2
...
# Create PagerDuty escalation policy for the Authentication team
resource "pagerduty_escalation_policy" "authentication_escalation_policy" {
  count     = can(var.teams["authentication_team"]) ? 1 : 0
  name      = "Authentication Team Escalation Policy"
  num_loops = 2
  teams     = [can(pagerduty_team.teams["authentication_team"]) ? pagerduty_team.teams["authentication_team"].id : null]

  dynamic "rule" {
    for_each = pagerduty_schedule.authentication_schedule
...
resource "pagerduty_schedule" "authentication_schedule" {
  count     = can(var.teams["authentication_team"]) ? 1 : 0
  name      = "Daily Authentication Rotation"
  time_zone = "Asia/Kolkata"  # Set the time zone to Indian Standard Time (IST)
  ...
  layer {
    ...
    rotation_virtual_start       = "2024-02-16T00:00:00+05:30"  # Adjust the virtual start time according to IST
    rotation_turn_length_seconds = 86400
    users                        = can(pagerduty_team.teams["authentication_team"]) ? [for user in pagerduty_user.authentication_team_users : user.id] : []
  ...
  teams = can(pagerduty_team.teams["authentication_team"]) ? [pagerduty_team.teams["authentication_team"].id] : []
}
...
locals {
  escalation_policies = {
    ...
    "Growth Team" = can(pagerduty_team.teams["engineering_team"]) ? pagerduty_escalation_policy.engineering_escalation_policy[0].id : null
    "Authentication Team" = can(pagerduty_team.teams["authentication_team"]) ? pagerduty_escalation_policy.authentication_escalation_policy[0].id : null
